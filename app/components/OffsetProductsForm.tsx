	...
	return (
		<>
			<fetcher.Form action='/offset-products-transaction' className='mb-4 flex w-full flex-wrap sm:mx-auto sm:max-w-screen-lg sm:items-end' method='post'>
				...
				<div className={fetcher.state === 'submitting' ? 'pointer-events-none w-full text-center opacity-50 sm:ml-4 sm:hidden' : 'w-full text-center sm:ml-4 sm:hidden'}>
					<Button text={content.cta.next} type='submit' />
				</div>
				<div className={fetcher.state === 'submitting' ? 'pointer-events-none hidden opacity-50 sm:ml-4 sm:flex sm:h-14 sm:w-2/12' : 'hidden sm:ml-4 sm:flex sm:h-14 sm:w-2/12'}>
					<Button text={content.cta.next} type='submit' />
				</div>
			</fetcher.Form>
		</>
